...
class MBMPO:
    def __init__(self, obs_dim, act_dim):
        ...
        self.dyn_opts = [
            optim.Adam(self.dyn.models[i].parameters(), lr=1e-3)
            for i in range(self.ensemble_size)
        ]

        self.inner_lr = 1e-3 

    def train_dynamics(self, real_data, epochs=40, batch_size=256):
        # Convert real_data transition tuples into tensors
        ...
        for i in range(self.ensemble_size):
            ...
            for epoch in range(epochs):
                # Shuffle dataset for each epoch to break sequence dependencies
                ...
                for start_idx in range(0, dataset_size, batch_size):
                    ...
                    b_act = act_t[batch_indices]
                    b_targets = delta_targets[batch_indices]
                    
                    # Compute forward pass delta prediction
                    x = torch.cat([b_obs, b_act], dim=-1)
                    pred_delta = self.dyn.models[i](x)  
                    loss = ((pred_delta - b_targets) ** 2).mean()

                    # Use the specific optimizer isolated for model index i
                    self.dyn_opts[i].zero_grad()
                    loss.backward()
                    self.dyn_opts[i].step()
                    
                    epoch_loss += loss.item()
                    num_batches += 1
                    
            # Compute average loss over all optimization updates for this ensemble member
            total_loss += (epoch_loss / num_batches)

        # Return the collective mean loss across the entire ensemble
